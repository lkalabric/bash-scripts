...
for i in $(find ${DEMUXCATDIR} -type f -exec basename {} .fastq \; | sort); do
	...
	for ((j=1; j<=6; j++)); do
		echo -e "\nExecutando filter_by_start_time $STAR_TIME[${j}]..."
		egrep -A3 "${START_DATE}T${REGEXP[$j]}" "${DEMUXCATDIR}/${i}.fastq" > "${FILTER_BY_START_TIMEDIR}/${i}.${STAR_TIME[${j}]}.fastq"
		echo -e "\nContando as reads do arquivo "${FILTER_BY_START_TIMEDIR}/${i}.${STAR_TIME[${j}]}.fastq"..."
		wc -l "${FILTER_BY_START_TIMEDIR}/${i}.${STAR_TIME[${j}]}.fastq"
	done
done
